...
import { Camera, CheckCircle, AlertTriangle, ArrowRight, Cpu, Zap, Activity, ShieldCheck, Microchip, Eye, Gauge, Terminal, PlaySquare, Hexagon, Download } from 'lucide-react';
...
function App() {
  ...
  const [graphData, setGraphData] = useState<number[]>(Array(40).fill(50));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setGraphData(prev => {
        ...
        newData.push(Math.max(0, Math.min(100, nextVal)));
        return newData;
      });
    }, 150);
    return () => clearInterval(interval);
  ...
  const exportReport = () => {
    if (!diagnosis) return;
    const content = `SNAPDRAGON AI LAB CHALLENGE - DIAGNOSTIC REPORT
TIMESTAMP: ${new Date().toISOString()}
NPU LOAD: ${Math.round(npuLoad)}%
LATENCY: 12.4ms

SCENARIO: ${current.name}
FAULT DETECTED: ${isGood ? 'NONE' : diagnosis.possible_fault}
REASONING: ${diagnosis.why}
RECOMMENDED ACTION: ${diagnosis.next_step}

-- GENERATED BY SNAPCIRCUIT LAB EDGE AI --`;
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'snapcircuit_diagnostic.txt';
    a.click();
      ...
      <main className="max-w-[1800px] mx-auto px-6 lg:px-10 py-6 w-full flex-1 flex flex-col relative z-10">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 relative">
          ...
          <div className="lg:col-span-4 bg-slate-900/40 border border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-2xl backdrop-blur-xl relative group hover:border-white/20 transition-all duration-500">
            ...
            <div className="px-5 pb-5 mt-auto flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[10px] font-bold text-slate-500 tracking-[0.15em] flex items-center gap-2">
                  <PlaySquare size={12} />
                  SIMULATION CONTROLS
                </h3>
                <button onClick={exportReport} disabled={loading || !diagnosis} className="flex items-center gap-1.5 px-3 py-1 rounded bg-sky-500/10 border border-sky-500/20 text-sky-400 text-[9px] font-bold hover:bg-sky-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-[0_0_10px_rgba(14,165,233,0.1)]">
          ...
          <div className="lg:col-span-4 bg-slate-900/40 border border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-2xl backdrop-blur-xl relative group hover:border-white/20 transition-all duration-500">
            ...
            {/* Telemetry Graph */}
            <div className="h-16 w-full bg-black/20 border-t border-white/5 relative overflow-hidden flex items-end">
              <div className="absolute top-2 left-3 flex items-center gap-1">
                <Activity size={10} className="text-slate-500" />
                <span className="text-[8px] font-mono text-slate-500">LIVE TELEMETRY</span>
              </div>
              <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
                <polyline 
                  points={graphData.map((val, i) => `${(i / (graphData.length - 1)) * 100},${100 - val}`).join(' ')}
                  fill="none" 
                  stroke={loading ? '#38bdf8' : isGood ? '#10b981' : '#ef4444'} 
                  strokeWidth="2" 
                  strokeLinecap="round" 
                  strokeLinejoin="round" 
                  className="transition-colors duration-500 drop-shadow-[0_0_5px_currentColor]"
                />
              </svg>
            </div>
